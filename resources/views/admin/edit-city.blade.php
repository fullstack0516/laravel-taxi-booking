                ...
                <div class="row">
                    <div class="col-xl-12">
                        <div class="dashboard-box">
                            <div class="headline">
                                <h3><i class="icon-line-awesome-building"></i> Edit City</h3>
                            </div>
                            <div class="content" style="padding: 20px 30px">
                                @if($errors->any())
                                    ...
                                    @endforeach
                                @endif
                                <form class="row" action="{{ route('cities.update', ['city' => $city->id]) }}" method="post" id="city-form">
                                    @method('put')
                                    @csrf
                                    <div class="col-xl-6">
                                        <div class="submit-field">
                                            <h5>City Name</h5>
                                            <input type="text" class="with-border" name="name" value="{{ old('name', $city->name) }}" required autofocus>
                                        </div>
                                        <div class="submit-field">
                                            <h5>Address</h5>
                                            <input type="text" id="address" class="with-border" name="address" value="{{ old('address', $city->address) }}" required>
                                        </div>
                                        <div class="submit-field">
                                            <h5>Country</h5>
                                            <input type="text" id="country_code" class="with-border" name="country_code" value="{{ old('country_code', $city->country_code) }}" required>
                                        </div>
                                        <div class="submit-field">
                                            <h5>Background Image</h5>
                                            <div class="keyword-input-container">
                                                <input type="text" class="keyword-input" id="background_image" name="background_image" value="{{ old('background_image', $city->background_image) }}" placeholder="Background Image" required>
                                        ...
                                        <input type="hidden" name="latitude" id="latitude" value="{{ old('latitude', $city->location->getLat()) }}">
                                        <input type="hidden" name="longitude" id="longitude" value="{{ old('longitude', $city->location->getLng()) }}">
                                        <input type="hidden" name="postal_code" id="postal_code" value="{{ old('postal_code', $city->postal_code) }}">
                                    </div>
                                    <div class="col-xl-6">
